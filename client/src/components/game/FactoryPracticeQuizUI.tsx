...
export function FactoryPracticeQuizUI() {
  ...
  const handleAnswer = useCallback(
    ...
    [answeredCorrectly, animating, question.correctIndex, addFactoryPracticeScore, hadWrongAttempt, incrementFirstTry, addPopup]
  );
  ...
  if (showSummary) {
    return (
      ...
      >
            ...
            return (
              <svg
                ...
                style={{
                  ...
                  animation: summaryPhase >= 0 ? `factory-star-pop-${star} 0.5s ease-out ${star * 0.2}s both` : "none",
        ...
        <style>{`
          @keyframes factory-star-pop-1 {
            0% { transform: scale(0) rotate(-30deg); opacity: 0; }
            60% { transform: scale(1.3) rotate(5deg); opacity: 1; }
            100% { transform: scale(1) rotate(0deg); opacity: 1; }
          }
          @keyframes factory-star-pop-2 {
            0% { transform: scale(0) rotate(-30deg); opacity: 0; }
            60% { transform: scale(1.3) rotate(5deg); opacity: 1; }
            100% { transform: scale(1) rotate(0deg); opacity: 1; }
          }
          @keyframes factory-star-pop-3 {
            0% { transform: scale(0) rotate(-30deg); opacity: 0; }
            60% { transform: scale(1.3) rotate(5deg); opacity: 1; }
            100% { transform: scale(1) rotate(0deg); opacity: 1; }
          }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 16 }}>
        {question.options.map((option, i) => {
          ...
          if (answeredCorrectly && i === question.correctIndex) {
            ...
            borderColor = "#4caf50";
          } else if (selectedAnswer === i && wrongAttempt) {
      ...
      {!answeredCorrectly && hadWrongAttempt && (
        <div
          style={{
            padding: "12px 16px",
