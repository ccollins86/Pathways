...
const THEME: QuizTheme = {
  ...
  accentColor: "#69f0ae",
  accentColorRgba: "rgba(105, 240, 174, 0.2)",
  ...
  animationPrefix: "ocean-confetti",
};
...
export function OceanPracticeQuizUI() {
  ...
  const questions = useMemo(() => {
    if (oceanQuestions && oceanQuestions.length > 0) return oceanQuestions;
    return [...FALLBACK_QUESTIONS].sort(() => Math.random() - 0.5);
  }, [oceanQuestions]);

  const actions: QuizActions = useMemo(() => ({
    close: closeOceanPractice,
    addScore: addOceanPracticeScore,
    complete: completeOceanPractice,
    resetScore: () => useGame.getState().resetOceanPracticeScore(),
    closeOnFinish: false,
  }), [closeOceanPractice, addOceanPracticeScore, completeOceanPractice]);
