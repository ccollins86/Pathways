...
- 2026-03-03: Integrated LLM-generated practice questions using OpenAI (gpt-5-mini via Replit AI Integrations). Both Town (if/else) and Ocean (loops) quizzes now fetch fresh AI-generated questions when the player enters each world. Questions pre-load in the background so they're ready by quiz time. Hardcoded questions kept as fallback if API is unavailable. New API endpoint: POST /api/generate-questions (topic: "if-else" | "loops"). Store fields: townQuestions, oceanQuestions, townQuestionsLoading, oceanQuestionsLoading, preloadTownQuestions(), preloadOceanQuestions().
