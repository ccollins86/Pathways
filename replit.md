...
## Recent Changes
- 2026-03-27: TEMPORARY - Login disabled (bypassed with Guest user). Commented-out code marked with TEMPORARY in App.tsx for easy re-enable.
- 2026-03-27: Added unified scoring system - totalScore accumulates across all worlds; +10 base per correct answer, +5 first-try bonus (no wrong attempts), +25 quest completion bonus per world, +50 world practice completion bonus; ScoreHUD component (top-right star badge with animated score counter and popups); PracticeQuizBase enhanced with first-try tracking, animated "+5 First Try!" badges, and beautiful completion summary screen (animated rows: correct answers, first-try bonuses, world completion, session total); FactoryPracticeQuizUI updated with same scoring features; all scores reset on game restart
