...
## Recent Changes
- 2026-03-07: Visual polish pass for town world - varied tree types (pine with layered canopy + round leafy trees with clustered foliage, color variation), multi-sphere bushes, clustered rocks, colorful flower patches (18 clusters with varied colors), 8 street lamps with warm point lights along roads, improved roads with sidewalks and dashed lane markings (cleared at intersection), wooden fence around house yard with walk-through gap, animated drifting clouds in sky, warmer golden-hour lighting with fill light, park benches, blue mailbox near house, pond with lily pads, dirt patches near house for ground variation; road exclusion zones prevent objects spawning on roads; house at [-12, 0, 10]
...
- 2026-03-17: Extended LLM question generation to Factory World (functions topic). All 3 quiz worlds now fetch fresh AI-generated questions. API endpoint supports topics: "if-else", "loops", "functions". Store fields: factoryQuestions, factoryQuestionsLoading, preloadFactoryQuestions(). Questions preload when entering Factory World.
- 2026-03-03: Integrated LLM-generated practice questions using OpenAI (gpt-5-mini via Replit AI Integrations). Town (if/else), Ocean (loops), and Factory (functions) quizzes fetch fresh AI-generated questions when the player enters each world. Questions pre-load in the background so they're ready by quiz time. Hardcoded questions kept as fallback if API is unavailable. New API endpoint: POST /api/generate-questions (topic: "if-else" | "loops" | "functions"). Store fields: townQuestions, oceanQuestions, factoryQuestions + loading states + preload functions.
...
- `client/src/components/game/PracticeQuizBase.tsx` - Shared quiz component (hints, retry-on-wrong, confetti) used by all practice quizzes
- `client/src/components/game/PracticeQuizUI.tsx` - Town practice quiz (if/else questions), uses PracticeQuizBase
- `client/src/components/game/OceanPracticeQuizUI.tsx` - Ocean practice quiz (loops questions), uses PracticeQuizBase
- `client/src/lib/random.ts` - Secure random utilities (secureRandom, secureShuffle) using crypto.getRandomValues
