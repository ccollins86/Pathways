...
## Recent Changes
- 2026-03-19: Added Psychic Shop (World 4) - 3-round search algorithm teaching game; Round 1: random guessing, Round 2: sequential/linear search (enforced +1 guessing), Round 3: binary search (enforced formula guess=(min+max)/2); win/lose thresholds ($200/$200 for R1/R2, $300 for R3); transition screens between rounds ("there has to be a better way"); final lesson comparing random/linear/binary search with code example; PsychicWorld.tsx (3D psychic shop), PsychicInstructionsUI, PsychicTransitionUI, PsychicRoundResultUI, PsychicLessonUI, PsychicGuessingUI (customer dialogue "Is it X?" → "Less/Greater than X"), PsychicHUD (balance bar, round label); cha-ching sound on correct guesses; portal in Factory World after practice quiz
...
### Psychic Shop (World 4) Flow — 3-Round Search Algorithm Teaching
...
4. **Round 1 (Random Search)**: Player guesses any number randomly. Win at +$200, lose at -$200.
5. **Round 2 (Linear/Sequential Search)**: Pick a starting number, then must guess sequentially (+1 each time). Hints if non-sequential. Win at +$200, lose at -$200.
6. **Round 3 (Binary Search)**: Must start at 50, then follow `guess = floor((min + max) / 2)`. Hints if wrong formula. Win at +$300 (nearly impossible to lose).
7. After each round win: "You won, but there has to be a better way" transition screen
8. After each round loss: "Unfortunately, you lost!" retry screen
9. After Round 3 win: lesson overlay comparing random vs linear vs binary search
10. Cha-ching sound (success.mp3) on correct guesses, success sound on round wins
11. Customer dialogue shows "Is it X?" → "Less than X" / "Greater than X" / customer says "Aw man! You won!"
12. Balance progress bar in HUD shows distance to win/lose thresholds
