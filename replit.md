...
## Recent Changes
- 2026-04-03: Added admin skip buttons for learning & practice phases - Hardcoded admin username list (admin, teacher, instructor, demo) in `client/src/config/adminUsers.ts`; `AdminSkipPanel` component in GameHUD.tsx with "Learning Phase" and "Practice Phase" skip buttons; admin panels integrated into all 3 worlds (Town via GameHUD, Ocean via World2AdminPanel, Factory via World3AdminPanel); reserved admin usernames blocked at registration in `server/auth.ts`; works in production (not gated by IS_DEV)
- 2026-03-20: Replaced hardcoded practice quiz questions with LLM-generated questions - All 3 practice quizzes (conditionals, loops, functions) now fetch fresh questions from `/api/generate-questions/:topic` using OpenAI via Replit AI Integrations; server-side generation with retry logic (up to 3 attempts), robust JSON parsing with sanitization, strict 8-question validation; client checks `/api/llm-status` to detect LLM availability — if unavailable (e.g. Render deployment without API keys), all quizzes use hardcoded fallback questions from `client/src/data/fallbackQuestions.ts`; `useGeneratedQuestions` hook serves shuffled fallbacks immediately, blends in LLM questions for unserved positions; preloading on world entry; FactoryPracticeQuizUI refactored to use shared `PracticeQuizBase`
